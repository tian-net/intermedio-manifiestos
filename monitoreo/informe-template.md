...
| 4 | Dashboard MongoDB muestra "No data" | MongoDB Exporter sin collectors habilitados (solo `general`) | Agregar `args` al sidecar con `--collect-all --compatible-mode` | `mongo-cluster/mongo-deployment.yaml` |
| 5 | Pod MongoDB sidecar en `Error: unknown flag` | Nombres de collectors incorrectos (`--collector.database`, `--collector.connections`) | Cambiar a `--collect-all --compatible-mode` (nombres reales: `dbstats`, `currentopmetrics`) | `mongo-cluster/mongo-deployment.yaml` |
| 6 | Dashboard sigue mostrando "No data" en MongoDB y JVM | Nombres de métricas en queries del dashboard no coinciden con los reales del exporter y el backend | Corregir 7 queries en dashboard: `mongodb_database_collection_size_bytes`→`mongodb_dbstats_dataSize{database="elchino"}`, `jvm_memory_used_bytes`→`jvm_memory_committed_bytes`, `job`→`application="Elchino"`. P99 sin datos porque WebFlux no expone histogram buckets → cambiar a promedio | `monitoreo/grafana/grafana-dashboard-configmap.yaml` |
| 7 | `git push` rechazado (`fetch first`) | Cambios remotos no sincronizados | `git pull origin develop` y resolver merge conflict en `comandos.md` | `comandos.md` |
| 8 | Merge conflict en `comandos.md` | Versión local vs remota divergentes en sección Frontend | Resolver manteniendo ambas versiones (MAQUINA LOCAL + EC2-3) | `comandos.md` |
| 9 | Minikube warning: "memory allocation leaves no room for system overhead" | t3.small (2GB RAM) muy justo para 3072MB default de Minikube | Reducir memoria: `minikube start --memory=2048mb` (opcional, warning no bloqueante) | — |
